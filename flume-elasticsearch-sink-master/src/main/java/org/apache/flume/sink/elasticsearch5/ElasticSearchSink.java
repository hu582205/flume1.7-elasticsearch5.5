...
import org.apache.flume.sink.elasticsearch5.client.ElasticSearchClient;
import org.apache.flume.sink.elasticsearch5.client.ElasticSearchClientFactory;
	...
	public Status process() throws EventDeliveryException {
		...
		try {
			...
			for (count = 0; count < batchSize; ++count) {
				...
				client.addEvent(event, indexNameBuilder, StringUtils.isEmpty(realIndexType)
						? ElasticSearchSinkConstants.DEFAULT_INDEX_TYPE : realIndexType, ttlMs);
			}
	...
	public void configure(Context context) {
		if (!isLocal) {
			if (StringUtils.isNotBlank(context.getString(ElasticSearchSinkConstants.HOSTNAMES))) {
				serverAddresses = StringUtils.deleteWhitespace(context.getString(ElasticSearchSinkConstants.HOSTNAMES))
						.split(",");
			}
		...
		if (StringUtils.isNotBlank(context.getString(ElasticSearchSinkConstants.TTL))) {
			...
			Preconditions.checkState(ttlMs > 0, ElasticSearchSinkConstants.TTL + " must be greater than 0 or not set.");
		}
